    ...
    inquirer.prompt(options).then(async (res) => {
      if ("View all departments" === res.options) {
        ...
        const [results] = await db.promise()
          .query(`SELECT role.title, role.id, role.salary, department.name AS department_id
                    FROM  role
        ...
        const [results] = await db.promise()
          .query(`SELECT employee.id, employee.first_name, employee.last_name, role.salary, role.title AS role, department.name AS department, CONCAT(manager.first_name, "_", manager.last_name) AS manager
                    FROM employee
        ...
        const [results] = await db
          .promise()
          .query(`INSERT INTO department(name) VALUES (?)`, res.newDepartment);
        console.table(results);
        return init();
        ...
        const [manager] = await db
          .promise()
          .query(
            `SELECT CONCAT(first_name, "_", last_name) AS name, manager_id AS value FROM employee`
          ); // query not working
        const res = await inquirer.prompt([
          {
            type: "input",
        ...
        const [employee] = await db
          .promise()
          .query(
            `SELECT CONCAT(first_name, "_", last_name) AS name, id AS value FROM employee`
          );
        const [role] = await db
          .promise()
          .query(`SELECT title AS name, id AS value FROM role`);
